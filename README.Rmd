...
Each are described in a separate vignette. 
...
txtbar <- function(x) {
  ...
  #max_nchar <- max(y)
  #y4 <- lapply(y3, function(z) {
    #z_pad <- paste0(rep.int(" ", max_nchar - nchar(z)), collapse = "")
    #paste0(z, z_pad)
  #})
  ret <- data.frame(name = names(x), value = unlist(y3))
